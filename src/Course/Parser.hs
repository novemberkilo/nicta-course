...
bindParser f p =
  P (\s -> case parse p s of
        ErrorResult e -> ErrorResult e
        Result i a -> parse p' i where p' = f a)
...
list1 ::
  ...
  -> Parser (List a)
list1 p =
  -- flbindParser p (f :: a -> Parser (List a))
  flbindParser p
    (\k -> -- flbindParser Parser (List a) (g :: List a -> Parser (List a))
      flbindParser (list p) (\k' -> valueParser (k :. k')))
...
satisfy ::
  ...
  -> Parser Char
satisfy f =
  bindParser (\c -> if f c then valueParser c else unexpectedCharParser c) character
...
sequenceParser (p :. pl) =
  bindParser (\k -> bindParser (\k' -> valueParser (k :. k')) (sequenceParser pl)) p
...
firstNameParser ::
  Parser Chars
firstNameParser =
  bindParser (\k -> bindParser (\k' -> valueParser (k :. k')) (list lower)) upper
...
surnameParser =
  flbindParser upper
    (\k ->
      flbindParser (thisMany 5 lower)
      (\k' ->
        flbindParser (list lower)
        (\k'' ->
          valueParser (k :. (k' ++ k'')))))
  -- bindParser (\k -> bindParser (\k' -> bindParser (\k'' -> valueParser (k :. (k' ++ k''))) (list lower)) (thisMany 5 lower)) upper
...
personParser =
  flbindParser ageParser
    (\k ->
      flbindParser (spaces1 >>> firstNameParser)
        (\k' ->
          flbindParser (spaces1 >>> surnameParser)
            (\k'' ->
              flbindParser (spaces1 >>> smokerParser)
                (\k''' ->
                  flbindParser (spaces1 >>> phoneParser)
                    (\k'''' ->
                      valueParser
                        Person {
                            age = k
...
instance Applicative Parser where
  ...
  (<*>) ::
    ...
    -> Parser b
  p <*> a =
    flbindParser p (\k -> flbindParser a (\k' -> valueParser (k k')))
